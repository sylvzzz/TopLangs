    ...
    <section className="container mx-auto px-6 py-20">
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center gap-3 mb-10">
          ...
          <h2 className="text-4xl font-bold text-[#15054f]">{username} Top Languages</h2>
        </div>

        <div className="relative">
          <div className="absolute -inset-1 bg-linear-to-r /*from-indigo-500 via-purple-500 to-pink-500*/ rounded-2xl blur opacity-20" />
          <div className="relative bg-white/5 border border-white/10 p-8 rounded-2xl backdrop-blur-sm">
            {loading && (
              <p className="text-[#15054f] text-sm animate-pulse">Fetching languages from GitHub...</p>
            )}
            ...
            {!loading && !error && (
              <>
                <div className="grid grid-cols-3 gap-4 mb-8">
                  {[
                    ...
                    <div key={label} className="bg-white/5 border border-white/10 rounded-xl p-4">
                      <p className="text-xs font-bold text-[#15054f] mb-1">{label}</p>
                      <p className="text-xl font-bold text-[#15054f]">{value}</p>
                    </div>
                  ))}
                </div>
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-2 mb-8">
                  {langs.map(([lang], i) => (
                    <div key={lang} className="flex items-center gap-1.5 text-xs font-semibold text-[#15054f]">
                      <span
                  ...
                  {langs.map(([lang, bytes], i) => {
                    ...
                    return (
                      <div key={lang} className="flex items-center gap-3">
                        <span className="text-xs text-[#15054f] w-4 text-right shrink-0">{i + 1}</span>
                        <span className="text-sm font-semibold text-[#15054f] w-28 shrink-0">{lang}</span>
                        ...
                        <span className="text-xs text-[#15054f] w-10 text-right shrink-0">{pct}%</span>
                      </div>
                    );
                  })}
                </div>
